...
- カテゴリ内の対象 1 件の失敗は、既存オーケストレーターの結果(翻訳された ID / スキップされた ID / 出力の有無)から判定する。
...
成功・失敗・スキップの件数が完了画面に表示され、失敗した対象が記録されること。`[失敗した項目を再試行]` により失敗対象だけに絞った計画で再実行され、新しい `sessionId` が使われること。
...
- 失敗対象だけに絞った再試行計画が組み立てられ、新しい `sessionId` で実行されること
