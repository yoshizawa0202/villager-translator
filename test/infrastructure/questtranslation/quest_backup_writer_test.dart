...
void main() {
  ...
  test(
    '選択した SNBT ファイルが logs/localizer/{sessionId}/backup/snbt_original/ 配下へ、'
    '相対パス構造を維持したままバックアップされる(受け入れ条件6)',
    () async {
      final original = File(
        p.join(
          tempDir.path,
      ...
      final backedUpFile = File(
        p.joinAll([backupDirectory.path, 'config', 'ftbquests', 'quests', 'chapter', 'a.snbt']),
      );
